...
class OllamaClient:
    ...
    def format_prompt_for_option2_analysis(self, query: str, context: str, chat_history: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        prompt_parts.append("=== MULTI-SOURCE CONTEXT FOR ANALYSIS ===")
        prompt_parts.append("You have access to THREE distinct types of information retrieved through similarity search:")
        prompt_parts.append("")
        prompt_parts.append("1. FORENSIC HOST IDENTIFICATION DATA: Critical information about infected hosts")
        prompt_parts.append("   - IP addresses, MAC addresses, hostnames, user accounts, domains, OS versions")
        prompt_parts.append("   - This data comes from multiple protocol layers (DHCP, ARP, NetBIOS, Kerberos, SMB)")
        prompt_parts.append("   - Look for sections labeled 'INFECTED HOST IDENTIFICATION' or 'FORENSIC INVESTIGATION'")
        prompt_parts.append("   - When asked about infected host attributes, PRIORITIZE this forensic data")
        prompt_parts.append("")
        prompt_parts.append("2. NETWORK TRAFFIC SEGMENTS: Detailed packet-level data from the PCAP file")
        ...
        prompt_parts.append("")
        prompt_parts.append("3. VIRUSTOTAL THREAT INTELLIGENCE: Pre-analyzed security threat data")
        ...
        prompt_parts.append("")
        prompt_parts.append("=== FORENSIC INVESTIGATION GUIDANCE ===")
        prompt_parts.append("When asked about infected host attributes (IP, MAC, hostname, user account):")
        prompt_parts.append("")
        prompt_parts.append("1. CHECK FORENSIC METADATA FIRST:")
        prompt_parts.append("   - Look for 'INFECTED HOST IDENTIFICATION' or 'FORENSIC INVESTIGATION NARRATIVE' sections")
        prompt_parts.append("   - These sections consolidate all host identification data in one place")
        prompt_parts.append("   - They include Q&A format answers for common questions")
        prompt_parts.append("")
        prompt_parts.append("2. UNDERSTAND PROTOCOL SOURCES:")
        prompt_parts.append("   - MAC Address: Found in ARP table or Ethernet layer data")
        prompt_parts.append("   - Hostname: Found in DHCP Option 12, NetBIOS names, or SMB native OS")
        prompt_parts.append("   - User Account: Found in Kerberos authentication or SMB sessions")
        prompt_parts.append("   - Domain: Found in DHCP Option 15, Kerberos realm, or SMB domain")
        prompt_parts.append("   - OS Information: Found in SMB native_os field or DHCP vendor class")
        prompt_parts.append("")
        prompt_parts.append("3. CITE DATA SOURCES:")
        prompt_parts.append("   - Always mention where the information came from (e.g., 'According to DHCP Option 12')")
        prompt_parts.append("   - This builds confidence and shows thoroughness")
        prompt_parts.append("")
        ...
        prompt_parts.append("- For forensic questions (IP, MAC, hostname, user), check FORENSIC sections FIRST")
        prompt_parts.append("- CORRELATE findings between forensic data, network traffic, and threat intelligence")
        prompt_parts.append("- CITE specific evidence: mention protocol sources (DHCP, ARP, Kerberos, SMB)")
